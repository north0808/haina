...
import com.haina.beluga.webservice.flexjson.locators.StaticClassLocator;
...
public class ObjectBinder<T> {
     ...
     * List<Person> pes = new ArrayList<Person>();
		pes.add(person);
		pes.add(person);
		String s = serializer.include("address").serialize(pes);
		System.out.println(s);
		List<Person> person1 = new JSONDeserializer<List<Person>>().use(null, Person.class).deserialize(s);
     */
    private Object bindArray( List input ) {
    	List list = new ArrayList();
    	for(int i = 0 ; i < input.size() ; i ++ ){
    		list.add(bindObject( (Map)input.get(i) ));
    	}
        return list;
    }
