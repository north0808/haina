		  ...
		  const gchar *symbol_name)
{
  ...
  gpointer p;
   wchar_t *filename;
   filename = g_utf8_to_utf16 (symbol_name, -1, NULL, NULL, NULL);
  if (handle == null_module_handle)
    {	  
      if ((p = GetProcAddress (GetModuleHandle (NULL), filename)) == NULL)
	p = find_in_any_module (symbol_name);
    }
  else
    p = GetProcAddress (handle, filename);
