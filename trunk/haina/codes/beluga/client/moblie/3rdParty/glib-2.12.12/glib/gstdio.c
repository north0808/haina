...
/*
int
g_access_old (const gchar *filename,
	  int          mode)
{
#ifdef G_OS_WIN32
...
//scott wince port , return 0 if successful
int
g_chmod (const gchar *filename,
	 int          mode)
{
 ...
 */
 /* 
int
g_open_old (const gchar *filename,
	int          flags,
	int          mode)
{
#ifdef G_OS_WIN32
	 ...
	 int          mode)
{
	...
	return g_open(filename,O_CREAT,0);
	  ...
	  const gchar *newfilename)
{
   ...
   if (G_WIN32_HAVE_WIDECHAR_API ())
      ...
      else
	{
	  ...
	  switch (GetLastError ())
	    {
#define CASE(a,b) case ERROR_##a: save_glib_errno = b; break
	  ...
	  const gchar *newfilename)
{
      ...
      if (wnewfilename == NULL)
	{
      ...
      if (MoveFile(woldfilename, wnewfilename))
	retval = 0;
      else
	{
	  retval = -1;
	 ...
	 int          mode)
{
 ...
 if (1)
      ...
      if (wfilename == NULL)
	{
      ...
      retval = CreateDirectory (wfilename,0);
      save_glib_errno = glib_errno;
 ...
 * glib_errno value. rmdir() is tried regardless of what caused remove() to
 * fail. Any glib_errno value set by remove() will be overwritten by that
 * set by rmdir().
 *
...
/*  
int
g_remove_ (const gchar *filename)
{
#ifdef G_OS_WIN32
if (G_WIN32_HAVE_WIDECHAR_API ())
    {
...
{
#ifdef G_OS_WIN32
scott if (G_WIN32_HAVE_WIDECHAR_API ())
    {
      wchar_t *wfilename = g_utf8_to_utf16 (filename, -1, NULL, NULL, NULL);
      int retval;
      int save_glib_errno;
...
if (1)
      ...
      if (wfilename == NULL)
	{
      ...
      retval = RemoveDirectory (wfilename);
      save_glib_errno = glib_errno;
